...
import type { EngineOutput, ConvergencePoint, DivergencePoint, ConfidenceBreakdown } from '../../engine/types.js'
import type { ExtractedClaim } from '../../extraction/types.js'
import type { EvidenceItem, ClaimEvidenceLink } from '../../engine/types.js'
...
export function extractVarianceDriver(
  ...
  const divergenceThemes = safeDivergencePoints.map(dp => (dp?.theme_label || '').toLowerCase()).filter(Boolean)
  ...
  return safeDivergencePoints[0]?.theme_label || 'Uncertainty in key assumptions'
}
...
export function computeEvidencePosition(
  ...
  if (claimsExtracted > 0 && evidenceLinksTotal === 0) {
    return {
      ...
      unsupportedCount,
      institutionalInterpretation: 'Claims derive from primary stimulus but lack external corroboration.',
    }
  }
...
export function extractConvergenceField(
  ...
  return (convergencePoints || []).map(cp => ({
    theme: cp?.theme_label || 'Unknown theme',
    interpretation: `Cross-lens agreement on ${(cp?.theme_label || 'unknown').toLowerCase()}. ${(cp?.supporting_lenses || []).length} lenses converge.`,
  }))
}
...
export function extractActiveDivergence(
  ...
  return (divergencePoints || []).map(dp => ({
    theme: dp?.theme_label || 'Unknown divergence',
    positions: (dp?.positions || []).map((pos: any, idx: number) => ({
      label: pos?.position_summary || `Position ${idx + 1}`,
